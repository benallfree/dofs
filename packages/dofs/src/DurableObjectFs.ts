...
export class DurableObjectFs extends DurableObject {
	...
	private mountFsApi() {
		this.ctx.storage.fs = {
			readFile: (path: string, options?: ReadFileOptions) => {
				const ino = this.resolvePathToInodeSync(path);
				const cursor = this.ctx.storage.sql.exec('SELECT offset, data, length FROM chunks WHERE ino = ? ORDER BY offset ASC', ino);
			...
			writeFile: (path: string, data: ArrayBuffer | string, options?: WriteFileOptions) => {
				...
				this.ctx.storage.sql.exec('INSERT INTO chunks (ino, offset, data, length) VALUES (?, ?, ?, ?)', ino, 0, buf, buf.length);
			},
			...
			write: (path: string, data: ArrayBuffer | string, options: WriteOptions) => {
				...
				this.ctx.storage.sql.exec('DELETE FROM chunks WHERE ino = ?', ino);
				const buf = typeof data === 'string' ? new TextEncoder().encode(data) : new Uint8Array(data);
				this.ctx.storage.sql.exec('INSERT INTO chunks (ino, offset, data, length) VALUES (?, ?, ?, ?)', ino, offset, buf, buf.length);
			},
			mkdir: (path: string, options?: MkdirOptions) => {
				...
				const parent = this.resolvePathToInodeSync(parentPath);
			...
			rename: (oldPath: string, newPath: string) => {
				...
				const oldParent = this.resolvePathToInodeSync(oldParentPath);
				const newParent = this.resolvePathToInodeSync(newParentPath);
				const oldCursor = this.ctx.storage.sql.exec('SELECT ino FROM files WHERE parent = ? AND name = ?', oldParent, oldName);
				const oldRow = oldCursor.next().value;
				if (!oldRow) throw new Error('ENOENT');
				const ino = oldRow.ino;
				// If destination exists, delete it
				const newCursor = this.ctx.storage.sql.exec('SELECT ino FROM files WHERE parent = ? AND name = ?', newParent, newName);
	...
	private async resolvePathToInode(path: string): Promise<number> {
		if (path === '/' || path === '') return 1;
		const parts = path.split('/').filter(Boolean);
		let parent = 1;
		for (const name of parts) {
			const cursor = await this.ctx.storage.sql.exec('SELECT ino FROM files WHERE parent = ? AND name = ?', parent, name);
			const row = cursor.next().value;
	...
	private async allocInode(): Promise<number> {
		const cursor = await this.ctx.storage.sql.exec('SELECT MAX(ino) as max FROM files');
		const row = cursor.next().value;
	...
	private resolvePathToInodeSync(path: string): number {
		if (path === '/' || path === '') return 1;
		const parts = path.split('/').filter(Boolean);
		let parent = 1;
		for (const name of parts) {
